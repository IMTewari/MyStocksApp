...
export function CoachSummary({
  ...
  const riskCount = Object.values(flags).flat().filter((f: any) => f.severity === "risk").length;
        ...
        <div style={{ textAlign:'right' }}>
          ...
          {top.length > 0 && (
            <div style={{ fontSize:13, color:'#555' }}>
              Top weights: {top.map(t => `${t.sym} ${Math.round((t.value/Math.max(total,1))*100)}%`).join(' · ')}
            </div>
          )}
        </div>
      </div>
      <div style={{ marginTop:8 }}>
        ...
        <ul style={{ marginTop:6, paddingLeft:18, fontSize:13 }}>
          {Object.entries(tips).map(([sym, t]) => (
            <li key={sym}><b>{sym}</b>: {t.action.toUpperCase()} — {t.reason} ({t.confidence})</li>
          ))}
          {Object.keys(tips).length === 0 && <li>No actions yet — awaiting data.</li>}
        </ul>
      </div>
    </section>
  );
